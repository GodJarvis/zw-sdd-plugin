...
<!-- 条件性段落：异步任务/可观测性如果只涉及本模块，放这里而非全局 -->
...
| 操作 | 路径 | 职责 |
|------|------|------|
| 新增 | `library/services/XxxService.php` | <!-- 职责 --> |
| 修改 | `apps/admin/controllers/XxxController.php` | <!-- 职责 --> |
...
**文件**: `library/services/XxxService.php`
**参照**: `library/services/MemberService.php`

```php
class XxxService
{
    public function methodA(int $param): array
...
**文件**: `apps/admin/controllers/XxxController.php`
**参照**: `apps/admin/controllers/MemberController::detailAction`
**约束**: 标准信封返回，参数校验走 Validation 类

#### RedisKey / CacheKey 常量扩展（签名级示例）

<!-- 条件性：涉及 Redis 新增/修改 Key 时 MUST 保留 -->

**文件**: `library/constants/RedisKey.php`（静态 Key）
**文件**: `library/modules/CacheKey.php`（动态 Key）
**参照**: 现有常量结构
...
{
    /** 绑卡操作锁前缀 */
    public const LOCK_BIND_CARD = 'lock:bindCard';
}
...
class CacheKey
{
    ...
    public static function loginRate($ip, $date)
}
```

**实现要点**:
- 无动态参数的 Key 放 `RedisKey`，含动态参数的放 `CacheKey`
- `CacheKey` 方法内通过 `self::$prefix` 拼接前缀
